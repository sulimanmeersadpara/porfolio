...
const projects = [
  ...
  },
  {
    ...
    demo: "https://salmanmeer.pythonanywhere.com/",
    highlights: [
  ...
  return (
    <PortfolioShell title="Projects" eyebrow="Selected work">
      <div className="space-y-8">
        <SectionTitle eyebrow="Featured work" title="Delivering practical, polished products for modern businesses." description="These projects reflect my focus on strong architecture, clean UI, and dependable implementation across full-stack web products." />
        <div className="grid gap-6 xl:grid-cols-4 lg:grid-cols-2">
          {projects.map((project, index) => (
            <motion.div key={project.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} whileHover={{ y: -8, scale: 1.01 }} className="group overflow-hidden rounded-[32px] border border-[#C9A25A]/20 bg-[#111111]/90 shadow-[0_20px_90px_rgba(0,0,0,0.25)] backdrop-blur-xl">
              <div className="relative h-56 overflow-hidden bg-gradient-to-br from-[#111111] via-[#1d1d1d] to-[#6F4E1F]">
                ...
                <div className="absolute inset-0 m-6 rounded-[24px] border border-[#C9A25A]/20 bg-black/30 backdrop-blur-sm" />
              </div>
              <div className="p-6">
